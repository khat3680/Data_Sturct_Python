"""
-------------------------------------------------------
Hash_Set_sorted list version of the Hash Set ADT.
-------------------------------------------------------
...
class Hash_Set:
    ...
    def _find_slot(self, key):
        ...
        h_key = hash(key) % self._slots
        slot = self._table[h_key]
        return slot
    ...
    def _rehash(self):
        ...
        while len(temp_table) > 0:
            temp_slot = temp_table.pop(0)
            
            while not temp_slot.is_empty() :
                value = temp_slot.remove_front()
                slot = self._find_slot(value)
                slot.insert(value)
