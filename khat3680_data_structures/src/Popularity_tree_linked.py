...
class Popularity_Tree:
    ...
    def _retrieve_aux(self, node, key):
        ...
        if value == node._value and node is not None:
            node._rcount +=1
            
        if node._rcount > node._left._rcount:
            self._rotate_left(node)
            
        elif  node._rcount > node._right._rcount:
    ...
    def _rotate_right(self, parent):
        ...
        parent._left = updated._right
        updated._right = parent._
        return updated
